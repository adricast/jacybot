...
from IAlib.IAlib.as_textpreprocesing import preprocess_rnn,preprocess_text
...
model_file = 'bot11_rnn_model.h5'
weights_file = 'bot11_rnn_model_weights.h5'
tokenizer_file = 'bot11_tokenizer.pkl'
label_encoder_file = 'bot11_label_encoder.pkl'
...
def salir(user_input):
     if user_input.lower() == 'salir':
            confirmacion = input("¿Seguro que quieres salir? (s/n): ").lower()
            if confirmacion == 's':
                confirmacion2 = input("¿cuando salgas deseas borrar la configuracion? (s/n): ").lower()
                if confirmacion2 == 's':
        ...
        if tipo_usuario in ['entrenador', 'usuario']:
            if tipo_usuario == 'entrenador':
                ...
                if (not entrenadores_df.empty and
                    (entrenadores_df['usuario'] == nombre_usuario) & (entrenadores_df['contrasena'] == contraseña)).any():
                    ...
                    return tipo_usuario,nombre_usuario
                else:
...
def get_response_tfidf(user_input, vectorizer, clf, data, data_file, variables, configuracion):
    ...
    if max_similarity_entry < entry_similarity_threshold:
        ...
        if not configuracion.empty and 'tipo' in configuracion.columns:
            ...
            if tipo_usuario == 'entrenador':
                qst_addresponse = input("¿desas ayudarme? (s/n): ")
                if qst_addresponse.lower() == 's':
                    ...
                    print("Chatbot: Te podria ayudar en algo mas.")
                    return None, None, None
            else:
...
def get_response_rnn(user_input, tokenizer, rnn_model, label_encoder, data, data_file, variables, configuracion):
    try:
        ...
        if configuracion['tipo'].iloc[0] == 'entrenador':
            ...
            if feedback == 'no':
                reentrenar = input("¿Deseas reentrenar el modelo? (s/n): ").lower()
                if reentrenar == 's':
                    tokenizer, label_encoder, rnn_model = train_rnn_model(data['entrada'], data['salida'], max_sequence_length=4)
                    save_rnn_model(rnn_model, tokenizer, label_encoder, weights_file, tokenizer_file, label_encoder_file)
...
def main():
    ...
    if configuracion.empty:
        ...
        tipo_usuario,nombre_usuario = obtener_tipo_usuario()
